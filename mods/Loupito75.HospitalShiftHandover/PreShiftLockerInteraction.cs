...
{
    internal static class PreShiftLockerInteraction
    {
        ...
        private sealed class InteractionState
        {
            ...
            internal InteractionPhase Phase;
        }
        ...
        private static readonly Dictionary<EmployeeComponent, InteractionState> States =
            new Dictionary<EmployeeComponent, InteractionState>();
            ...
            EmployeeComponent employee)
        {
            ...
            return state.Phase != InteractionPhase.WaitingForAccessClear;
        }
            ...
            EmployeeComponent employee)
        {
            ...
            return States.TryGetValue(employee, out state) &&
                   state != null &&
                   state.Stamp == stamp &&
                   (state.Phase == InteractionPhase.ClosingLocker ||
                    state.Phase == InteractionPhase.WaitingForAccessClear);
        }
            ...
            EmployeeComponent employee)
        {
                ...
                state.Stamp == stamp)
            {
                ...
                if (walk != null &&
                    state.Phase == InteractionPhase.GoingToLocker)
                {
                    walk.Stop();
                }
            ...
            string reason)
        {
            ...
            if (States.TryGetValue(employee, out active) &&
                active != null &&
                active.Stamp == stamp &&
                active.Phase != InteractionPhase.WaitingForAccessClear)
            {
                return;
            }
            ...
            if (reason == "shift-started" &&
                IsAtWorkplaceForDressingFallback(employee))
            {
                RestoreProfessionalClothes(
                    employee,
                    "shift-started-at-workplace");
                CompletedStamps[employee] = stamp;
                WorkplaceFallbackStamps.Remove(employee);
                Log(
                    employee,
                    "FALLBACK_COMPLETE",
                    "trigger=shift-started-at-workplace");
                return;
            }
            ...
            float activitySlackMinutes)
        {
            ...
            if (state == null)
            {
                ...
                state = new InteractionState
                {
                    ...
                    Phase = InteractionPhase.GoingToLocker
                };
            ...
            if (state.Phase == InteractionPhase.WaitingForAccessClear)
            {
                return false;
            }

            if (state.Phase == InteractionPhase.GoingToLocker)
            {
                ...
                state.Phase = InteractionPhase.OpeningLocker;
                Log(
                    employee,
                    "ACTIVATE",
                    "locker=" + GetObjectId(activeLocker) +
            ...
            if (state.Phase == InteractionPhase.OpeningLocker)
            {
                if (!HasCurrentAnimationFinished(activeAnim))
                {
                    return true;
                ...
                state.Phase = InteractionPhase.UsingLocker;
                Log(
                    employee,
                    "OPEN",
                    "locker=" + GetObjectId(activeLocker) +
            ...
            if (state.Phase == InteractionPhase.UsingLocker)
            {
                if (!HasCurrentAnimationFinished(activeAnim))
                {
                    return true;
        ...
        private static bool IsAtWorkplaceForDressingFallback(
            EmployeeComponent employee)
        {
            if (employee == null ||
                employee.m_entity == null)
            ...
            BehaviorDoctor doctor =
                employee.m_entity.GetComponent<BehaviorDoctor>();
            if (doctor != null)
            {
            ...
            BehaviorNurse nurse =
                employee.m_entity.GetComponent<BehaviorNurse>();
            if (nurse != null)
            {
            ...
            BehaviorLabSpecialist lab =
                employee.m_entity.GetComponent<BehaviorLabSpecialist>();
            ...
            int modulo)
        {
            ...
            unchecked
            {
                ...
                // Mix the decision channel as a full input before the final avalanche
                // so independent dressing decisions do not share modulo patterns.
